...
import groq # importar la librería Groq para el modelo LLM
...
# Inicializar cliente Groq de manera eficiente
@st.cache_resource
def init_groq_client():
    """Inicializa cliente Groq de manera eficiente"""
    try:
...
# CAMBIO 3: Nueva función para obtener respuestas de Groq
def obtener_respuesta_groq(mensaje_usuario, contexto=""):
    """Obtiene respuesta inteligente de Groq para el contexto médico"""
    if not groq_client:
        return None
    
    try:
        # Prompt especializado para citas médicas
        system_prompt = """Eres un asistente médico virtual de Clínica MediCare en Guadalajara, México.
        
SERVICIOS DISPONIBLES:
- Consulta General: $500 MXN (Dr. Rodríguez)
- Pediatría: $600 MXN (Dr. García)  
- Cardiología: $800 MXN (Dra. Martínez)
- Dermatología: $700 MXN (Dr. López)
- Laboratorio: $250 MXN (QFB Angel Carrizalez)

HORARIOS: Lunes-Viernes 9:00-18:00, Sábados 9:00-11:30

INSTRUCCIONES:
- Sé empático y profesional
- Ayuda con citas, precios, horarios y información médica básica
- Si es emergencia, recomienda llamar al 911 o (33) 1234-5678
...
- Usa emojis médicos apropiados: 🏥 👨‍⚕️ 📅 💰 ⏰ 🚨"""
        ...
        mensajes = [
            ...
            {"role": "user", "content": f"Contexto previo: {contexto}\n\nConsulta: {mensaje_usuario}"}
        ]
...
# CAMBIO 4: Modificar la función generar_respuesta
def generar_respuesta_con_groq(mensaje, intencion):
    """Genera respuestas combinando tu lógica actual + Groq"""
    
    # Para casos complejos que tu chatbot ya maneja bien, usar tu lógica
    if intencion in ["procesar_cita_completa", "cancelar_cita", "cambiar_cita", 
                     "buscar_cita", "solicitar_cancelacion", "solicitar_cambio"]:
        # Usar tu función original
        respuesta_original = generar_respuesta(mensaje, intencion)
        return respuesta_original
    
    # Para casos más generales, intentar primero con Groq
    contexto_previo = ""
    if len(st.session_state.messages) > 1:
        # Tomar últimos 2 mensajes como contexto
        contexto_previo = " | ".join([
            f"{msg['role']}: {msg['content'][:100]}..." 
            for msg in st.session_state.messages[-2:]
        ])
    
    respuesta_groq = obtener_respuesta_groq(mensaje, contexto_previo)
    
    if respuesta_groq:
        # Si Groq responde, usar su respuesta + MENSAJE DE GUÍA
        mensaje_guia = """

---
📋 **Para un mejor servicio, proporcióname tus datos como en estos ejemplos:**
...
**📅 ¿Listo para agendar tu cita?** ¡Solo dame todos los datos juntos! ⚡"""

        return f"🤖 **Respuesta con IA Avanzada:**\n\n{respuesta_groq}{mensaje_guia}"
    else:
        # Si Groq falla, usar tu lógica original como respaldo
        return generar_respuesta(mensaje, intencion)
            ...
            with st.chat_message("assistant"):
                with st.spinner("🤖 Procesando con IA y consultando base de datos..."):
                    try:
                        ...
                        respuesta = generar_respuesta_con_groq(prompt, intencion)  # ← NUEVA LÍNEA
                        st.markdown(respuesta)
